...
def lstmTrain(batchSz):
    ...
    ## FOr Testing
    logdirTesting = "tensorboard/romney_" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S") + "Testing/"
    writerTesting = tf.summary.FileWriter(logdirTesting)
    writerTesting.add_graph(sess.graph)

    # ### For the training
    logdirTraining = "tensorboard/romney_" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S") + "Training/"
    writerTraining = tf.summary.FileWriter(logdirTraining)
    writerTraining.add_graph(sess.graph)


    logdirModel = "models/romney_" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S") + "/"
